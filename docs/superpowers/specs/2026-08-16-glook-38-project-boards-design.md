...
`project_key`, `active_status` and `middle_status` are interpolated into quoted JQL literals, so all three are validated on write: project keys against `/^[A-Z][A-Z0-9_]*$/`, status names rejected if they contain a double quote **or a backslash**. Both characters matter — JQL string literals escape with backslash, so a status name ending in one would escape the closing quote and let the literal run on into the rest of the query. Status names stay on a two-character denylist rather than a positive whitelist because real Jira statuses contain spaces, ampersands and parentheses (SPS has `Specs & Design` and `Ready for Dev`), and a character class would reject valid configuration. Validation lives in one module, and `buildProjectJql` re-checks at the point of use — the same defence-in-depth the superseded revision's final review asked for.
