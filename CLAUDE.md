...
- **Jira project board config** (`src/lib/jira-projects/`) — the `jira_projects` table is org-scoped, one row per project shown on the `/projects` board: project key, display name, active/middle status, hierarchy (`goal-initiative` or `owner`), position. `src/lib/jira-projects/` owns validation, JQL construction (`buildProjectJql`), and the one-time legacy-JQL seed. The active and middle tabs must query the exact named status, never a status category — on production Jira, `status = "In Progress"` returns 46 SPS epics vs. `statusCategory = "In Progress"`'s 71 (the extra 25 sit in Discovery, Rollout, Specs & Design, Ready for Dev), so category inference would silently inflate the board. This is the single thing a future change to this area is most likely to break.
...
- Jira project keys and status names (`jira_projects` table) are interpolated into quoted JQL literals, so both are validated at save time (`validateJiraProject`) **and** re-validated at point of use in `buildProjectJql` — the latter catches rows written directly to the DB, bypassing the Settings form. Status validation is deliberately a **denylist** (rejects `"`, `\`, newlines) rather than a whitelist, because real statuses contain `&` — e.g. `Specs & Design`. Do not "tighten" it to a whitelist.
