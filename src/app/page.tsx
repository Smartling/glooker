...
export default function Home() {
  ...
  // Auto-load the latest completed report on mount
  useEffect(() => {
    setLoading(true);
    fetch('/api/llm-config')
      .then((r) => r.json())
      .then((config) => {
        if (config.latestReport?.id) {
          return fetch(`/api/report/${config.latestReport.id}`).then((r) => r.json());
        }
        return null;
      })
      .then((data) => {
      ...
      .catch((err) => console.error('[glooker] Failed to load latest report:', err))
      ...
      <div className="flex-1 min-w-0">
        ...
        {!loading && activeReport && (
          <div className="flex items-center justify-between mb-4">
            <div>
              <span
                className="text-gray-300 font-medium hover:text-indigo-400 cursor-pointer transition-colors"
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {teams.length > 0 && (
                <select
                  ...
                  className="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-400 focus:outline-none focus:border-indigo-500 cursor-pointer"
                >
              ...
              {[...filterLogins].map(login => {
                ...
                return (
                  <span key={login} className="inline-flex items-center gap-1.5 bg-indigo-500/20 text-indigo-300 text-xs font-medium px-2.5 py-1 rounded-lg border border-indigo-500/30">
                    {dev?.avatar_url && <img src={dev.avatar_url} alt="" className="w-4 h-4 rounded-full" />}
                    {dev?.github_name || login}
                    <button onClick={() => setFilterLogins(prev => { const n = new Set(prev); n.delete(login); return n; })} className="text-indigo-400 hover:text-white ml-0.5">&times;</button>
                  </span>
                );
              })}
                  ...
                  return (
                    <>
                      <input
                        ...
                        className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 w-48"
                      />
                      {matches.length > 0 && (
        ...
        {!loading && activeReport && developers.length === 0 && activeReport.status === 'completed' && (
        ...
        {!loading && !activeReport && (
          <div className="text-center text-gray-500 py-16">
            ...
            <p className="text-xs text-gray-600">
              Go to{' '}
              <a href="/reports" className="text-indigo-400 hover:text-indigo-300 transition-colors">
                Report History
              </a>
...
function AiPercentBadge({ value }: { value: number }) {
  const n = Number(value) || 0;
  if (n === 0) return <span className="text-gray-600 text-sm">&mdash;</span>;
              ...
              {commits.map((c: any) => (
                <tr key={c.commit_sha} className="border-b border-gray-700/30 last:border-0">
                  <td className="py-1.5 px-1 font-mono whitespace-nowrap align-top">
                    <a href={`https://github.com/${org}/${c.repo}/commit/${c.commit_sha}`} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 hover:underline">{c.commit_sha.slice(0, 7)}</a>
                  </td>
  ...
  const tooltip = show && typeof document !== 'undefined' ? createPortal(
    ...
    >
      ...
      {issues && issues.map((issue: any) => (
        ...
        >
          <span className="text-indigo-400 font-mono text-xs">{issue.issue_key}</span>
