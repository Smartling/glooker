      ...
      <table className="w-full text-sm table-fixed">
        <thead>
          <tr className="text-left text-xs text-gray-500 uppercase tracking-wider border-b border-gray-800">
            ...
            <th className="px-4 py-3 text-right w-[6%]" title="Cumulative team impact: sum of active developers' individual impact scores. Scales with team size; no saturation."><button onClick={() => onSort('impact_sum')} className="hover:text-gray-300">(Σ){sortCaret('impact_sum')}</button></th>
          </tr>
        </thead>
        <tbody>
          {sortedRows.map(row => (
            ...
            >
              ...
              <td className="px-4 py-3 text-right text-gray-400 tabular-nums">{row.impact_sum.toFixed(1)}</td>
            </tr>
          ))}
        </tbody>
      </table>
