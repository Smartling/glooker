...
async function getHandler(req: NextRequest, { params }: { params: Promise<{ id: string; login: string }> }) {
  ...
  try {
    ...
    // dev.ts orders this array by cost DESC for privileged viewers; once cost is
    // stripped, re-sort by model name so array order can't leak a relative-cost
    // ranking to someone who isn't allowed to see the amounts.
    if (!canSeeCost(result.developer.github_login)) {
      result.models = (result.models ?? [])
        .map(({ cost, ...rest }: any) => rest)
