...
export default function NavBar() {
  ...
  const isTeamActive = Boolean(isReportPage && (isDevDetail || !isOrgView));
  const isOrgActive = Boolean(isReportPage && isOrgView && !isDevDetail);
  ...
  return (
    <nav className="bg-[#111118] border-b border-gray-800/80 px-5 flex items-center h-12 gap-1 no-print">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Link href="/settings" className={`p-2 rounded-md transition-colors ${pathname.startsWith('/settings') ? 'text-indigo-400 bg-indigo-500/10' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800/50'}`}>
        ...
        {authEnabled && user && (
          <Link href="/profile" className={`flex items-center pl-1 pr-2.5 py-1 rounded-full transition-colors ${pathname.startsWith('/profile') ? 'bg-indigo-500/10' : 'hover:bg-gray-800/50'}`}>
            {user.avatarUrl ? (
